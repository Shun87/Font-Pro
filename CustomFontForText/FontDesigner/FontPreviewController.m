...
- (void)viewDidLoad
{
    ...
    textView1.font = [UIFont fontWithName:fontName size:25];
    ...
    button.showsTouchWhenHighlighted = YES;
    [button addTarget:self
               action:@selector(addSymbols:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(startAddSymbols:)
                                                 name:@"startAddSymbols"
                                               object:nil];
    ...
    UISegmentedControl *segment = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:NSLocalizedString(@"Share", nil),
                                                                             NSLocalizedString(@"Clear", nil),nil]];
    segment.segmentedControlStyle = UISegmentedControlStyleBar;
    segment.momentary = YES;
    self.navigationItem.titleView = segment;
    segment.tintColor = [UIColor colorFromHex:0x28282a];
    CGRect rc = segment.frame;
    rc.size.width += 30;
    segment.frame = rc;
    [segment release];
}

- (IBAction)addSymbols:(id)sender
{
    ...
    sysmbolNameViewController.showBackbutton = YES;
    [self.navigationController pushViewController:sysmbolNameViewController animated:YES];
}
...
- (IBAction)showProVersion:(id)sender
{
    ...
    UINavigationController *navigationController = [[UINavigationController alloc] initWithRootViewController:proViewController];
...
- (void)startAddSymbols:(NSNotification *)aNotification
{
    NSString *text = [aNotification object];
    ...
    {
        NSString *srcText = self.textView1.text;
        srcText = [srcText stringByAppendingFormat:@"%@", text];
        self.textView1.text = srcText;
    }
}
