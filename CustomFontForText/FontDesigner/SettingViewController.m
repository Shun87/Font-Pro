...
- (void)viewDidLoad
{
    ...
    self.appArray = nil;//[MoreApp moreApps];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 0)
    {
        if (row == 0)
        {
            [socila sendEmail:NSLocalizedString(@"TextFont", nil) body:NSLocalizedString(@"invite", nil) recipient:nil];
        }
        else if (row == 1)
        {
        ...
        {
            [socila sendFeedback:NSLocalizedString(@"TextFont", nil) body:nil];
        }
        else
        {
